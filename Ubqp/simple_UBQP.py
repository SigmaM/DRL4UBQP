...
class DRL4UBQP(nn.Module):
    def __init__(self, task_size, static_size, hidden_size, dynamic_size, num_layers, n_head, n_layers, k_dim,
        ...
        self.global_attention = GlobalAttentionEncoder(hidden_size, n_head, n_layers, k_dim,v_dim)
    ...
    def forward(self, Q, Q_DIAG, Q_RC, dynamic, decoder_input=None,
        ...
        idx = 0
        change = Q_DIAG[:, 0].float().to(device)
        state = change.ge(0).float()
        state[:, 0] = 0
        mask = state.clone()

        if mask.ne(0).all():
            mask[:, 0] = 1
        dynamic = torch.cat((state.unsqueeze(1), change.unsqueeze(1)), dim=1)
        del change
        del state
        change_t = torch.zeros(batch_size).to(device)
        x = torch.zeros(batch_size, sequence_size).to(device)
        x[:, 0] = 1
        reward_ = 0.
        for _ in range(max_steps):
            ...
            x[visit_idx_xy, ptr[visit_idx_xy]] = 1
