...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v):  # (n*b) x lv x dv
        ...
        attn = self.softmax(attn)  # n*b x lv x lv           (sum(2) =1(nb x lv) )
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, m_dim, k_dim, v_dim, dropout=0.1):
        ...
        # nn.init.normal_(self.w_qs.weight, mean=0, std=np.sqrt(2.0 / (m_dim + k_dim)))
        # nn.init.normal_(self.w_ks.weight, mean=0, std=np.sqrt(2.0 / (m_dim + k_dim)))
        # nn.init.normal_(self.w_vs.weight, mean=0, std=np.sqrt(2.0 / (m_dim + v_dim)))
    ...
    def forward(self, q, k, v):
        ...
        # output = self.layer_norm(input.view(-1, input.size(-1))).view(*input.size())
        return output
