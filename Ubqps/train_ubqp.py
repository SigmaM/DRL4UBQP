...
class StateCritic(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size):
        ...
        self.fc1 = nn.Conv1d(hidden_size * 1, 20, kernel_size=1)  # fliter256 x 1114  20个
...
def validate(data_loader, actor, RLLS, reward_fn, render_fn=None, save_dir='.', seed=1, num_plot=5):
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        #     device)  # b x 1 x n+1
        Q_diag = torch.cat((p0, Q_diag), dim=1).unsqueeze(1).to(device)
        Q_row = torch.cat((p0, Q_row), dim=1).unsqueeze(1).to(device)
...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    for epoch in range(4):  # epoch 5
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # Q_eigenvalues = torch.cat((p0, torch.from_numpy(Q_eigenvalues)), dim=1).unsqueeze(1).to(
            #     device)  # b x 1 x n+1
            Q_diag = torch.cat((p0, Q_diag), dim=1).unsqueeze(1).to(device)  # b x n+
            Q_row = torch.cat((p0, Q_row), dim=1).unsqueeze(1).to(device)
            dynamic = dynamic.to(device)
            Q_RC = Q_row
...
def activeSearch(actor, critic, task, num_nodes, test_data, reward_fn,
                 render_fn, batch_size, actor_lr, critic_lr, max_grad_norm, RLLS, epoch_num,
                 **kwargs):
    """Constructs the main actor & critic netw orks, and performs all training."""

    batch_size = 128
    test_loader = DataLoader(test_data, batch_size, True, num_workers=16)
    rewards_ = []
    time_ = 0
    for batch_idx, batch in enumerate(test_loader):
        ...
        for epoch in range(5):  # epoch 5
            ...
            epoch_idx += 1
        time1 = time.time()
        reward_ = torch.cat(rewards, dim=0).max().unsqueeze(0)
        rewards_.append(reward_)
        time_ += time1 - time0
    rewards_output = torch.cat(rewards_, dim=0).mean()
    return rewards_output,time_
...
def train_ubqp(args):
    ...
    if not args.instance:
        ...
        if args.checkpoint:
            ...
            path = os.path.join(args.checkpoint, 'critic.pt')
            critic.load_state_dict(torch.load(path, device))
            out, time1, out_ls, time_ls = validate(test_loader, actor, True, ubqp.reward, None, test_dir, test_seed,
                                                   num_plot=5)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--checkpoint', default="ubqp/200/1/checkpoints/9")
    #
    parser.add_argument('--checkpoint', default="ubqp/50/1")
